...
from flags.models import FlagState # Import actual FlagState for comparison in tests
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"verbosity": 1, "checks": False, "prompt": False, "static": True, "migrate": True, "debug": False, "admin_email": "", "admin_password": ""},
            True, True, True
        ),
        (
            {"verbosity": 0, "checks": True, "prompt": True, "static": False, "migrate": False, "debug": True, "admin_email": "test@test.com", "admin_password": "pw"},
            False, False, False
        ),
    ]
)
...
def test_handle_all_options_true_no_admin(mock_validate_email, mock_path, mock_os, mock_user, mock_logger, mock_env, mock_call_command):
    command = Command()
...
def test_handle_create_admin_user(mock_validate_email, mock_path, mock_os, mock_user, mock_logger, mock_env, mock_call_command):
    command = Command()
    command.stdout = MagicMock()
    command.style = MagicMock()
    ...
    mock_call_command.assert_has_calls([
        call("remove_stale_contenttypes", no_input=True, verbosity=0, stdout=command.stdout),
        call("createsuperuser", email="test@example.com", username="test@example.com", verbosity=0, interactive=False),
    ])
...
@patch("hope_documents.management.commands.upgrade.validate_email")
@override_settings(STATIC_ROOT="/tmp/static", FLAGS=[])
def test_handle_admin_user_exists(mock_validate_email, mock_path, mock_os, mock_user, mock_logger, mock_env, mock_call_command):
    command = Command()
    command.stdout = MagicMock()
    command.style = MagicMock()
